...
Implements the formulas from 01_PILLAR_1_POLICY_ECONOMICS/DEEP_RESEARCH.md:
...
Every response carries `assumptions` and `warnings` explicitly -- per the
Definition of Done, nothing is silently labelled "best" and no cost/GJ is
computed without a documented GCV.
"""
from __future__ import annotations

from app.models import BlendRequest, ScenarioRequest, SensitivityRequest
...
def compare_scenarios(scenarios: list[ScenarioRequest]) -> dict:
    ...
    # Rank only on a metric every scenario actually has (cost/GJ requires GCV on all of them).
...
def sensitivity_grid(req: SensitivityRequest) -> dict:
    """
    Required grid from DEEP_RESEARCH.md: Freight +-10%, FX +-5%, Port cost +-20%,
    GCV +-5%, Waiting days 0 / P50 / P90.
    Implemented as independent single-factor shocks (not a full cross-product)
    so the output stays a readable table for a jury demo; the cross-product
    variant is one loop away if Raghav wants the full cube later.
    """
